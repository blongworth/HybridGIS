...
bflow <- read.csv(here("Data/bubbleflow.csv")) %>%
  mutate(heflow = volume/time,
         ...
         calcflow = flowcalc(pressure, r, u, x)) # calculate flow with poiselles 
...
ldflow <- mutate(ldflow, sccm = Leakrate / 0.0016, # convert to sccm
                 ...
                 calcflow = flowcalc(Regulator, r, u, x)) # calculate flow with poiselles 
...
# mean of the solid OX-I's
stdrat <- mean(c(10.0038, 9.9445))

data.s <- data %>% 
  group_by(Pos) %>% 
  summarize(c1412.m = mean(X14.12he),
            c1412.s = sd(X14.12he),
            c1412.sdr = c1412.s/c1412.m,
            c1412.se = se(X14.12he),
            c1412.ser = c1412.se/c1412.m,
            cor1412.m = mean(cor1412he),
            counts = sum(CntTotGT),
            interr = 1/sqrt(counts)) %>% 
  mutate(normfm = cor1412.m/stdrat * 1.0398) 

knitr::kable(data.s, digits = 4,
             caption = "Summary data for US032817")
```
