...
data <- get_hgis_data(here("Data/USAMS053018R.txt"), 
           ...
           mutate(dil_factor = case_when(str_ends(Sample.Name, "He") ~ 1,
                                         TRUE ~ 0), 
                  Num = ifelse(Pos %in% c(3, 4), "S", Num),
                  normFm = norm_gas(cor1412he, mean(data$cor1412he[data$Num == "S"])))
sum_hgis(data)
```
...
Used positions 7-10, results in USAMS012220. Low currents: ~6uA pure CO2, 3uA diluted. Flows: He 10

```{r}
# Load data
data <- get_hgis_data(here("Data/USAMS012220R.txt"), 
                      as.Date(c("2020-09-10", "2020-09-29"))) %>% 
           mutate(dil_factor = case_when(str_starts(Sample.Name, "Dil") ~ 1,
                                         TRUE ~ 0), 
