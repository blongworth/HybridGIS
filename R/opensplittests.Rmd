...
3.5E-4 mbarls-1. This is `r` SCCM. 
...
data <- get_hgis_data(here("data/USAMS012121R.txt"), as.Date("2021-01-29")) %>% 
  ...
  group_by(Pos) %>% 
         mutate(outlier = ifelse(is.na(removeOutliers(cor1412he)), TRUE, FALSE))
```
...
ggplot(data, aes(cum_acqtime, he12C )) +
  geom_point(aes(color = outlier)) +
  facet_wrap(vars(pos_name), scales = "free")
```
...
First carbonate sample clogged capillary. Moved needle up to avoid sucking in acid. May need to consider a water trap. All samples measured on fresh targets after a 5 minute "burn in" to reduce blank.
...
It looks like the SD of modern samples is around 1%. Fm of NOSAMS 2 is a bit high. Need to investigate current-dependent blank.