...
Capillary break between Target blank and Helium blank. Replaced with SGE 100um x 52cm.
Initial currents of targets are high, between 10-25uA. Drops asymptotically to base target current in 5-10 minutes.
...
gas$pred = predict(fitlm)

ggplot(gas) +
  geom_line(aes(1/he12C, y = pred)) +
...
TODO: produce live/dead curves using helium dilution
...
Connected cage-split capillary to leak checker. Base leak rate with capillary in air: 1.7E-9 mbarLs-1. Leak rate with helium in open split: 3.5E-4 mbarls-1. This is `r  ` SCCM.
...
stdrat <- mean(df$cor1412he[df$pos_name == "8 - LiveGasOS"])
...
ds %>% 
  ...
  ggplot(aes(Pressure, Cur, color = Gas)) +
  geom_pointrange(aes(ymin = Cur - Cur.sd, ymax = Cur + Cur.sd)) + 
  geom_point(size = 4)
...
Same vs inverse current with fits.
...
ds %>% 
  filter(Pos %in% c(5, 6)) %>% 
  mutate(Pressure = str_extract(Sample.Name, "\\d+")) %>% 
  ggplot(aes(1/Cur, mean, shape = Pressure, color = Pressure)) +
  geom_pointrange(aes(ymin = mean - sd, ymax = mean + sd)) + 
  geom_point(size = 4)
```