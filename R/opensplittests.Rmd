...
Startup and test after opening tank to refill terminal argon bottle. Testing helium dillution by starting with helium in vial and displacing with CO2. Used XXm of 75um capillary to provide 100ulm to source, but initial source pressure of 3E-7 Torr indicates lower flows. Shortened split-cage capillary to about 150cm, without a huge effect on pressure. Flow may be defined by cage-source capillary.
...
Testing performance at higher dillution ratio. Started with pure live CO2 in vial and did long aquisition with high displacement helium flow to check current as CO2 concentration decreases. 
...
TODO: Predict mass flow of CO2 with dillution model
...
Use dilution function to determine expected CO2 in flow to source at time t.
...
All samples measured on fresh targets after a 5 minute "burn in" to reduce blank. 
...
carb_data <- tribble(~vial, ~name, ~rec_num, ~mass,
                     ...
                     2, "NOSAMS2_1", 38809, 32.3,
                     3, "NOSAMS2_1", 38809, 33.5,
                     4, "TIRI-I_1", 17185, 33.1,
                     5, "TIRI-I_1", 17185, 31.8,
                     6, "TIRI-I_1", 17185, 31.8,
                     7, "TIRI-F_1", 2138, 31.9,
                     8, "TIRI-F_1", 2138, 33.4,
                     9, "TIRI-F_1", 2138, 32.4
                     )
...
```{r}
# TODO: diagnose issue with Pos as factor not converting back to integer position.
...
```{r}
fmblank <- ds %>%
  filter(str_starts(Sample.Name, "DeadGas")) %>% 
  pull(mean) %>% 
  mean()
...
It looks like the normalization may be off a bit. Note positive linear correlation of Fm consensus difference with Fm of sample. Check how diff is done to confirm +- slope.
...
Using short capillaries from 2021-04-02. He flow set to 250ul/min.  Had to shorten delivery capilary at start of first C-2 due to acid clog. Needle blown out with compressed air.
