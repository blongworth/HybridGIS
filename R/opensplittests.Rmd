...
```{r}
data %>% 
  filter(str_detect(Sample.Name, "DeadGas|Blank")) %>% 
  ggplot(aes(1/he12C, normFm)) + 
  geom_point()
```
...
```{r}
ds <- sum_hgis(data) %>% 
  filter((str_detect(Sample.Name, "LiveGas") & Cur > 1) | str_detect(Sample.Name, "DeadGas|Blank")) %>% 
  mutate(Gas = ifelse(str_detect(Sample.Name, "LiveGas"), "LiveGas", "DeadGas"),
         Cur_inv = 1/Cur)
```
