...
.pull-right[
![HGIS Target](images/target_cutaway.svg)
...
read.csv(here("data/14Nov2017LPtest.csv"), skip = 4) %>%
  ...
  labs(title = "Current vs gas flow",
          ...
          y = "Source current (μA)") +
  theme_classic() +
  theme_xaringan(title_font_size = 26)
```
]
...
.pull-left[
Adapter between CO2 producers and source. 
...
* adding another gas increases pressure and total flow
* reduces pCO2
* CO2 remains constant? No- viscosity
]
...
P and V are held constant, so only mixture controls pCO2
...
Helium displacement of CO<sub>2</sub> from vial. Monitor currents and ratios as [CO<sub>2</sub>] decreases.

* No current suppresion at high CO<sub>2</sub>
* Current stable until 40 min
* Corresponds to 2mL CO<sub>2</sub>
...
ggplot(data, aes(co2flow, he12C)) +
  geom_smooth(span = .3, se=FALSE, color = medium_blue) +
  geom_point() +
...
$$Fm_m=Fm_bI_b+Fm_sI_s$$

$$Fm_s = Fm_m + \frac{I_b(Fm_m-Fm_b)}{I_m-I_b}$$

0.1uA blank current with 10uA total means 1% blank

Modeled blank:
* Fm: `r format(blank_val$Fm_blank, digits = 3)`
* Current: `r format(blank_val$m_blank, digits = 3)` uA

Measured blank:
* Fm: `r ds %>% filter(Gas == "Blank") %>% pull(mean) %>% mean() %>% format(digits = 3)`
* Current: `r ds %>% filter(Gas == "Blank") %>% pull(Cur) %>% mean() %>% format(digits = 3)`
...
.pull-left[
If standards, blanks, and unknowns have same current:
...
### Data summary
* mean difference: `r format(mean(df$fmbc_diff), digits = 1)`
* sd of difference: `r format(sd(df$fmbc_diff), digits = 2)`
...
Open split defines pressure, therefore

smaller samples = higher dilution = higher flow

plot: sample size vs. required flow

pressure control to maintain constant CO2 to source

plot: dilution modelling plot
...
* CO<sub>2</sub> from carbonates transferred onto resin trap in stream of he
...
NOSAMS has developed a successful method for analysing carbonates with an open-split interface delivering gas to a NEC MCSNICS gas-accepting Cesium sputter ion source.

Current goals:
* finish characterization of the method
* Comparison to graphite with test samples
* Improve accuracy
...
Mark Roberts, Josh Burton, Josh Hlavenka, Al Gagnon, Kalina Gospodinova, and the staff of NOSAMS
